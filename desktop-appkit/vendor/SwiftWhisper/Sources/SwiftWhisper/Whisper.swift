...
public class Whisper {
    ...
    public init(fromFileURL fileURL: URL, withParams params: WhisperParams = .default) {
        let contextParams = Whisper.makeContextParams()
    ...
    public init(fromData data: Data, withParams params: WhisperParams = .default) {
        ...
        self.whisperContext = copy.withUnsafeMutableBytes {
            whisper_init_from_buffer_with_params($0.baseAddress!, data.count, contextParams)
        }
        self.params = params
    }
    ...
    private static func makeContextParams() -> whisper_context_params {
        var params = whisper_context_default_params()

        // Newer upstream whisper.cpp defaults to GPU-enabled context params.
        // The local Swift wrapper currently vendors a CPU-only path first.
